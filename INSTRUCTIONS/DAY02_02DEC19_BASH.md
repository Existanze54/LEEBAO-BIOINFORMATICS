...
#### 1В. Управление командной строкой `BASH` путем нажатия клавиш
   ...
   `Ctr+D` – выход из сессии терминала  
   ...
   `Ctr+F` ИЛИ `->` – перемещение вперед на один символ  
   `Ctr+B` ИЛИ `<-`– перемещение назад на один символ  
   `Ctr+W` – удаление последнего слова  
   `Ctr+T` – поменять местами две соседние буквы  
   `Ctr+P` – предыдущая команда в истории  
   `Ctr+N` – следующая команда в истории  
